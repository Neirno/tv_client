...
class MainViewModel @Inject constructor(
    ...
    private fun turnDisplayOn() = intent {
        when (val result = powerUseCase.displayOn.invoke()) {
            ...
            is Result.Error -> postSideEffect(MainSideEffect.ErrorMessage(result.exception.message ?: "Error turning display on"))
        }
    }
    ...
    private fun turnDisplayOff() = intent {
        when (val result = powerUseCase.displayOff.invoke()) {
            ...
            is Result.Error -> postSideEffect(MainSideEffect.ErrorMessage(result.exception.message ?: "Error turning display off"))
        }
    }
    ...
    private fun turnLightOn() = intent {
        when (val result = powerUseCase.lightOn.invoke()) {
            ...
            is Result.Error -> postSideEffect(MainSideEffect.ErrorMessage(result.exception.message ?: "Error turning light on"))
        }
    }
    ...
    private fun turnLightOff() = intent {
        when (val result = powerUseCase.lightOff.invoke()) {
            ...
            is Result.Error -> postSideEffect(MainSideEffect.ErrorMessage(result.exception.message ?: "Error turning light off"))
        }
    }
